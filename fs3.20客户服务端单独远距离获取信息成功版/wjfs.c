...
ssize_t wjfs_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
	...
	// 	}
	// 	name[j][k] = '\0';
	// 	if(strcmp(name[0], "RDMA"))  //判断文件是否是远端的访问链接信息
	// 	{
	// 		is_rdma = 0;
	// 		break;
	// 	}
	...
	// {
	// 	wjfs_client(arg, name[1], base_port, message);
...
static int wjfs_client(void *arg, char *ip, int port, char *message)
{
...
/*for safe*/
	...
	addr_srv.sin_family = AF_INET;
	addr_srv.sin_port = htons(base_port);
...
//扫描全部的dentry拿到inode
void get_file_message(char *name, char *message)
{
	printk(KERN_EMERG"wjfs: %s ==>> %s ==>> %d\n",__FILE__,__FUNCTION__,__LINE__);
	printk("name : %s\n",name);
	...
	printk(KERN_EMERG"tmp_dentry->d_inode->i_ino %d\n",tmp_dentry->d_inode->i_ino);
	isize = i_size_read(a_dentry->d_inode);    //对应inode中的信息数量+1
	printk("isize %d\n",isize);
